...
class LoginWindow(tk.Toplevel):
    def __init__(self, parent, on_success):
        ...
        # UI Setup
        tk.Label(
            self, text="Enter GitHub Personal Access Token",
            bg=BG_MAIN, fg=ACCENT, font=FONT_BOLD,
        ).pack(pady=(20, 5))

        help_link = tk.Label(
            self, text="Where do I get a token?",
        ...
        self.token_entry = tk.Entry(
            self, bg=BG_STRIPE, fg=FG_TEXT, insertbackground=FG_TEXT,
            borderwidth=0, show="*", font=FONT_MAIN
        )
        self.token_entry.pack(fill=tk.X, padx=30, pady=20, ipady=8)

        self.btn_login = tk.Label(
            self, text="VERIFY & SAVE", bg=SUCCESS, fg="white",
            font=FONT_BOLD, pady=10, cursor="hand2"
        )
    ...
    def attempt_login(self):
        ...
        thread = threading.Thread(target=self.run_validation, args=(token,), daemon=True)
        thread.start()
    ...
    def handle_result(self, result):
        if result["success"]:
            ...
            self.btn_login.config(bg="#a83232", text=f"FAILED: {result['error']}", cursor="hand2")
            self.btn_login.bind("<Button-1>", lambda e: self.attempt_login())